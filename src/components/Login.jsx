...
const Login = () => {
 const [action, setAction] = useState("Sign Up")
  return (
     <section className="w-full h-fit  bg-[url('./assets/1481.jpg')] bg-cover bg-center">
    <span className="flex justify-center items-center p-2" >
   <div className="flex flex-col gap-4 justify-center items-center h-[400px] w-[420px] rounded-[40px] border border-purple-500 backdrop-blur-[6px]">
   <img className="w-9 h-9" src={logo} alt="Logo" />
    <div className="text-white text-[22px] font-bold pb-4">{action}</div>
<div className="flex flex-col gap-5">
 {action=== "Login"?<div></div>:<span className="flex flex-row items-center">
   <input className="relative outline-none bg-transparent text-white border border-purple-500 w-[350px] h-10 p-3 rounded-[40px] placeholder-white" type="text" placeholder="Username" />
   <FaRegUser className="absolute right-[10%] text-white" /> </span>}
 <span className="flex flex-row items-center ">
   <input className="relative outline-none bg-transparent text-white border border-purple-500 w-[350px] h-10 p-3 rounded-[40px] placeholder-white" type="email" placeholder="Email" />
   <MdEmail className="absolute right-[10%] text-white" />
 </span>
 <span className="flex flex-row items-center">
   <input className="relative outline-none bg-transparent text-white border border-purple-500 w-[350px] h-10 p-3 rounded-[40px] placeholder-white" type="password" placeholder="Password" />
